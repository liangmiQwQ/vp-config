...
According to these problems, we do not enable global `cache` in `vite.config.ts`. Instead, we automatically generate Vite tasks which's name starts with `c` for cacheable tasks as a pure wrapper. Personally, it may be a prepare and proof of concept of Vite+'s command level cache.
